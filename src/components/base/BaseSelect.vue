<template>
  <div ref="dropdownRef" class="relative">
    <label
      class="text-primary font-semibold flex items-end gap-x-1"
      @click="isOpen = !isOpen"
    >
      {{ displayedValue ?? placeholder }}

      <ChevronDownIcon
        class="w-4 h-4 mb-0.5 transition-transform"
        :class="{
          'rotate-180': isOpen,
        }"
      />
    ...
    <div
      class="z-10 bg-white divide-y divide-gray-100 rounded-lg shadow w-44 right-0"
      :class="{
        absolute: isOpen,
        hidden: !isOpen,
      }"
